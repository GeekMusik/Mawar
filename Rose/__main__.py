...
|         (C) 2021-2022 by @GeekMusik        | 
|          Greetings from Mawar  :)           |
|_____________________________________________|  
...
home_keyboard_pm = InlineKeyboardMarkup(
    [
        ...
        [
            InlineKeyboardButton(
                text="🌐 My Website",
                url=f"https://geekmusic.mhpay.repl.co",
            ),
            InlineKeyboardButton(
                text="🔰News Channel",
                url=f"https://t.me/GeekMusikUpdate",
            )
        ],
    ]
)
...
[👋]({random.choice(IMG)}) Hey there {message.from_user.mention}, 
   ...
   Nama saya Mawar, Bot manajemen grup telegram canggih Untuk bantuanAnda Melindungi Grup Anda & Sesuai Untuk Semua Kebutuhan Anda. 
Saat ini saya mengelola tentang `{len(served_chats)}` grup. Saya punya lebih `{len(served_users)}` pengguna

⚒ Kirimi aku /help Untuk Dapatkan Perintah. 
👨‍💻Developer : @mr_theherd
""",
...
async def startcq(client,CallbackQuery, _):
    ...
    await CallbackQuery.message.edit(
            text=f"""
👋 Hei disana {CallbackQuery.from_user.mention}, 

   Nama saya Rose, Bot manajemen grup telegram tingkat lanjut Untuk bantuan 
Anda Melindungi Grup Anda & Sesuai Untuk Semua Kebutuhan Anda. 
Saat ini saya mengelola tentang `{len(served_chats)}` grup. Saya punya lebih `{len(served_users)}` pengguna

 ⚒ Kirimi aku /help Untuk Dapatkan Perintah. 
👨‍💻Dᴇᴠᴇʟᴏᴘᴇʀ : @mr_theherd
""",
            disable_web_page_preview=True,
            reply_markup=home_keyboard_pm)
...
Saya bot manajemen grup dengan beberapa fitur yang berguna.
Anda dapat memilih opsi di bawah ini, dengan mengklik tombol.
Jika Anda memiliki bug atau pertanyaan tentang cara menggunakan saya,
lihat saya [Docs](https://geekmusic.mhpay.repl.co/mawar), atau menuju ke @MissMawar_bot.
**Semua perintah dapat digunakan dengan yang berikut:: / **""",
        keyboard,
    )
